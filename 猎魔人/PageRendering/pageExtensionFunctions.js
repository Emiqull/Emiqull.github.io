    ...
    document.querySelectorAll(PARAGRAPH_TAG).forEach(paragraph => {
        ...
        text = text.replace(QUOTE_SYMBOLS_REGEX, match => {
            switch (match) {
                case '“':
                case '’':
                    return `<span style="text-align: right; font-family: PingFang SC; display: inline-block; width: 1rem" class="${OPEN_QUOTE_CLASS}">${match}</span>`;
                case '”':
                case '‘':
                    return `<span style="text-align: left; font-family: PingFang SC; display: inline-block; width: 1rem" class="${CLOSE_QUOTE_CLASS}">${match}</span>`;
                default:
                    return match;
            }
        });
